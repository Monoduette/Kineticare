...
**KÖTELEZŐ SKILL: `.claude/skills/teszt-audit/SKILL.md`** (skill-név:
`teszt-audit`) — bármely teszt írása, módosítása, átnézése vagy átfésülése
előtt be kell tölteni. A felhasználói szintű, általános `test-audit` helyett
ez a projektváltozat az irányadó.
...
- Ellenpróba: a hibajavítás regressziós tesztje a javítás előtti kódon
  bizonyítottan, a szándékolt okból bukik, a javítással átmegy. A bukás
  üzenete a PR-leírás „hogyan lett ellenőrizve” részébe kerül.
- Megtartási mérce: minden önállóan őrzött szerződés (például pénz,
  NAV-számla, jogi szöveg, tilos zónák és őrtesztjeik, alapértelmezések,
  csomag- és platformőrök).
- A hálózati hívók injektálási pontjai (`postXml`, `queryByKulsoAzon`,
  `fetchImpl`) a 15. tanulság miatt maradnak; az audit nem törli őket csak
  tesztnek szóló varratként.
- Egy alrendszer teljes tesztfelületének megritkítása: `CAMPAIGN.md`
...
25. **A Barion `401 AuthenticationFailed` a bolt élesítése előtt nem
    ...
    `src/lib/barion/client.ts`). Az indítás csak az alakot nézi, ezért a futás
    közbeni 401 mögött formailag jó, de rossz kulcs áll: a titkos kulcs helyére
    került nyilvános kulcs (Public key), más bolt vagy más környezet kulcsa,
    vagy még nem élesített bolt (`BARION_START_OPERATOR_HINTS`). A döntő próba
    az élesítés utáni első indítás (runbook 11, pénzmozgás nélkül). Ha az is
    401, a kulcsot kell összevetni a Barion-fiók titkos kulcsával (Secret
    key).
