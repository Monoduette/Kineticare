 ...
 * stornózott számlák teljes összege, mínusz a kiállított helyesbítővel
 * lezárt részleges visszatérítések. Csak az a csökkentés számít, amelyhez
 * bizonylat is készült (Áfa tv. 153/B. § (1) a): az adóalap csökkentéséhez
 * az érvénytelenítő vagy módosító számla kell). Ez BECSLÉS és csak a
...
function partialRefundTotal(refunds: unknown): number {
  ...
  for (const entry of refunds) {
    if (typeof entry === 'object' && entry !== null) {
      ...
      if (record.type === 'partial') {
        total += finiteAmount(record.amountHuf)
      }
    }
  }
  return total
...
export function aamContribution(order: AamOrderInput, year: number): number {
  ...
  const deducted =
    order.correctiveInvoiceStatus === 'issued' ? partialRefundTotal(order.refunds) : 0
...
/** A `find`-hez kért mezők (a `refunds` a helyesbítős levonáshoz kell). */
