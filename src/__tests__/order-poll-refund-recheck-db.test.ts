...
// A hálózati próba ELŐTT ellenőrizzük a célpontot: az SQL-fixtúrák csak
// eldobható, helyi (localhost) CI-adatbázisba mehetnek (a név `_ci`-re végződik,
// a CI-ban `kineticare_ci`). Az audit:1 a hálózattiltott runner elérhetetlen jelzője.
if (process.env.DATABASE_URI) {
  const target = new URL(process.env.DATABASE_URI)
  if (
    !['localhost', '127.0.0.1', '[::1]'].includes(target.hostname) ||
    !(
      /^\/[a-z0-9_]+_ci$/.test(target.pathname) ||
      (target.pathname === '/audit' && target.port === '1')
    )
  ) {
...
describe.skipIf(!hasDb)('order-poll heti visszatérítés-újraellenőrzés (valódi PostgreSQL)', () => {
  ...
  it('a sávba eső teljes ÉS részleges visszatérítés GetState-et kap, a sávon kívüliek nem', async () => {
    const warns: string[] = []
    const log: Logger = {
      child: () => log,
      debug: () => undefined,
    ...
    // A közös CI-adatbázisban más tesztfájlok sorai is lehetnek: azokra
