...
await writeFile(path.join(directory, 'reset-minta.txt'), reset.text)
...
for (const [file, template] of [
  [
    ...
    freeCourseEmail({
      name: 'Őri Anna',
      email: 'ori.anna@example.com',
      courseTitle: 'Minta ingyenes kurzus',
      activationUrl: 'https://kineticare.example.com/jelszo-visszaallitas',
      expiresInDays: 7,
    }),
  ],
  [
  ...
  await writeFile(path.join(directory, `${file}.html`), template.html)
  await writeFile(path.join(directory, `${file}.txt`), template.text)
}
...
<nav>${examples.map((item) => `<a href="${item.file}.html">${item.label}</a><a href="${item.file}.txt">${item.label}: szöveges levél</a>`).join('')}<a href="celoldal.html">Átállási céllap</a><a href="reset-minta.html">Resetlevél, token nélkül</a><a href="ingyenes-belepo.html">Ingyenes kurzus belépőlevele</a><a href="fiok-aktivalas.html">Fiókaktiválás</a></nav>
