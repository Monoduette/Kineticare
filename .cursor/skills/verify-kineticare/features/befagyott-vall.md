...
- `shoulder-one-panel` has exactly one `section.kc-post-cta` root and one `section.kc-post-cta__panel`.
...
- **One panel.** `GET /blog/befagyott-vall` 200. Stripped HTML has exactly one `kc-post-cta` root and one `kc-post-cta__panel` section. Text includes `Hogyan tovább?`. Link `Kérj időpontot üzenetben` href is `/kapcsolat#idopontkeres`, once.
...
- Do not follow the appointment link into form submit on Railway. Proving the GET href is enough here; the form is the `kapcsolat-idopontkeres` feature.
