...
- Do not POST checkout, appointment, or contact forms on Railway. Do not GET `/api/checkout/start`. Do not seed, do not write CMS, do not touch env.
...
- Strip `<script>` and `<style>` before counting. Dual CTA is one `.kc-post-cta` root containing two sibling `.kc-post-cta__panel`. Do not treat two `.kc-post-cta` roots as a pass.
- Restore nothing: GET does not mutate. Still keep proof artifacts.

## Proof and skip reporting

- Capture the visitor GET and the HTML it produces. Product checkout proof is the product page GET href `/penztar?termek=N`, then GET of that form page. Never POST start. Callback proof is GET `/kapcsolat#idopontkeres`. Never submit.
...
- [Blog dual CTA](./blog-dual-cta.md) covers the seven kéz articles: one `.kc-post-cta` root, two sibling `.kc-post-cta__panel`.
