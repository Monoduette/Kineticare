...
| 40 | **A szolgáltatások listájára lépés** (kezdőlapi sín, Nyitott állapot) | `Nézd meg a szolgáltatásokat` | E/2 (P-1b) | `primary` | **Új sor (REV C, 2026-09-06).** A kattintás a `/szolgaltatasok` gyűjtőre visz: rendelő, otthoni program, szakmai út egy helyen. Puszta navigáció → E/2. A #10 (`Nézd meg a kurzusokat`) mintáját követi, más tárggyal: a cél oldal neve a felületen „Szolgáltatások”. **Forrás:** GOV.UK, *Add links* — *„If your link takes the user to a page where they can start a task, start your link with a verb"* (https://guidance.publishing.service.gov.uk/writing-to-gov-uk-standards/writing-guidelines/add-links/); NN/g, *Better Link Labels* — „Specific" (https://www.nngroup.com/articles/better-link-labels/); W3C, *WCAG 2.2 SC 3.2.4* (https://www.w3.org/WAI/WCAG22/Understanding/consistent-identification.html). |
| 41 | **Az Ingyenes SOS KézRelax oldalának megnyitása** (kezdőlapi sín, Zárt állapot) | `Ingyenes SOS KézRelax` | E/2 (P-1b) | `primary` | **Új sor (REV C, 2026-09-06).** A tulajdonos a terméknevet hagyta jóvá gombfeliratnak. A cél a SOS-kurzus oldala. **MERGE HOLD:** ugyanaz a `href`, mint a #3/#4 (`Elindítom ingyen`). A WCAG 2.2 **3.2.4** szerint azonos funkció azonos nevet kér; ezt a feszültséget emberi döntés zárja (egyesítés a #3-ra, vagy tudatos, dokumentált kivétel). Addig a kezdőlapi őr a két feliratot együtt tartja. **Forrás:** NN/g, *Better Link Labels* — a felirat a túloldalt nevezi meg (URL fent); GOV.UK, *Add links* (URL fent). |
